-- Migration: Add is_critical and min_score_required to evaluation_categories and update trigger
ALTER TABLE evaluation_categories 
ADD COLUMN IF NOT EXISTS is_critical BOOLEAN DEFAULT false,
ADD COLUMN IF NOT EXISTS min_score_required NUMERIC(3,2) DEFAULT 4.00;

-- Update calculate_evaluation_result to evaluate critical criteria at category average level
...
BEGIN
  ...
  FROM (
    SELECT jsonb_build_object(
      ...
      'min_required', ec.min_score_required
    ) AS fail_data
    FROM evaluation_answers ea
    JOIN evaluation_questions eq ON ea.question_id = eq.id
    JOIN evaluation_categories ec ON eq.category_id = ec.id
    WHERE ea.evaluation_id = p_evaluation_id
    GROUP BY ec.id, ec.name, ec.min_score_required, ec.is_critical
    HAVING ec.is_critical = true AND AVG(ea.score) < ec.min_score_required
  ) fails;

  v_has_critical_fails := (v_critical_fails IS NOT NULL AND jsonb_array_length(COALESCE(v_critical_fails, '[]')) > 0);

  -- Determinar resultado base por promedio
  ...
  -- Criterios críticos forzan resultado a 'plan_mejoramiento' sin importar el promedio
  IF v_has_critical_fails THEN
    v_pmi_required := true;
    v_result_status := 'plan_mejoramiento';
    IF v_pmi_reason <> '' THEN
  ...
  -- Marcar respuestas críticas fallidas (si aplica en base a categoría)
  ...
  WHERE ea.question_id = eq.id
    ...
    AND ec.is_critical = true
    AND (
      SELECT AVG(ea2.score)
      FROM evaluation_answers ea2
      JOIN evaluation_questions eq2 ON ea2.question_id = eq2.id
      WHERE ea2.evaluation_id = p_evaluation_id
        AND eq2.category_id = ec.id
    ) < ec.min_score_required;
